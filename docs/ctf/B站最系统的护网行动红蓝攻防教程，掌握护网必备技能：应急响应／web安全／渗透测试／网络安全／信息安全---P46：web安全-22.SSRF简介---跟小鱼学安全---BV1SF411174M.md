# B站最系统的护网行动红蓝攻防教程，掌握护网必备技能：应急响应／web安全／渗透测试／网络安全／信息安全 - P46：web安全-22.SSRF简介 - 跟小鱼学安全 - BV1SF411174M
...
但是你不能访问到这个公司它的一些内网的一些啊东西。因为它是有防火墙。🤧K。啊，我们其实可以看到这个SSF的话就是呃利用。诶。这样子的1个SSF漏洞的话，我们可以呃通过一些通过服务器。来去访问。

或者说去攻击内网的一个服务。🤧嗯。然后为什么他能够做到这样子的一个呃功能呢？🤧K。我们可以来。看一下这一个漏洞，它的一个形成的一个原因。大多的话它是由于服务端提供了从其他服务器应用获取数据的一个功能。

然后的话它没有对目标的一个地址做严格的过滤与限制。所以的话我们来呃理解一下这一个从其他服务器应用获取数据这样子的一个功能。这功能的话其实呃我们也是经常能够去见到，也也能够去使用到。就说呃。

比如说我们的一个服务器。呃。比如说我的呃我们的这样子的一个网站，对吧？我这里的一个网站。我访问我访问之后的话，我的比如说我这里的这一些图片，对吧？这图片的话在这里的话是放在呃我们的这一个服务器上面嘛。

如果说如果说你的这样子的一个图片，它是啊托管在其他的一些服务器上面。啊，他不是跟你的一个网站服务器，在一个服务器。所以的话他需要通过这种呃链接对吧？

通过一个链接的一个方式来去远程的去加载另外一个服务器上面，或者说图片服务器上面的这种资源。然后的话如果说他没有对呃这里的就说请求这一个图片资源的这一个呃链接。这种目标做一个限制以及过滤的话。

就会导致这样子的1个SSF的一个漏洞。我们可以利用呃我们可以利用我这里的这一个网站服务器。然后的话来去呃攻击利用其他的。呃，去访问其他的一个资源。然后这里的一个呃。这你的一个目标的一个地址的话。

我们可以。去使用到内网的一个地址。如果说他呃能够去允许去访问到内网的一些资源的话。好的话。这样子的一个过滤与限制，就是说它没有对我们的这样子的一个呃请求的这种资源，就是说请求其他服务器的这种资源啊。

这样这样子的一个链接，或者说这样子的一个UL做一个呃限制。它限制的话呃呃在后面的一个漏洞的一个修复，会有讲到，就是说呃比如说我们要限制它不能够去访问到内网的一些IP对吧？因为它它呃，因为它这一个服务器。

它去获取到的那些资源的话，它可能是存在内网的一个服务器上面的。所以说如果你要去获取到内网服务器的一个资源，你需要去通过这样子的一个链接，对吧？通过这样子的1个IP。

然后的话你没有对这样子的1个IP做这样子的一个限制。就是说呃因为你一个内网，你有其他的一些IP嘛，对吧？然后的话我们可以伪造这样子的一个就是伪造这样子的一个请求的一个目标。

由我们的这一个就是含有漏洞的这个服务器，向我们指定的这一个目标发送这样子的一个请求。比如说访问内网的一些IP的一些呃端口以及服务。

然后的话最常见的一个例子就是攻击者攻击者它会传入一个这种未经验证的1个UL。然后的话后端代码它直接去请求这样子的1个UL，然后的话会造成这样子的1个SSF漏洞。🤧嗯。就说呃我们的这里的1个UL的话。

是攻击者能够去可控的对吧？然后的话这一个呃UL可控的话。并并没有对他做一些过滤。以及限制。所以的话我们呃能够去进行一个利用，然后的话会导致这样子的一个漏洞。🤧う。

然后的话就是我们来呃区分一下SSR与CSR的一个区别。就说CSF我们其实上呃之前的课程的话讲到了，就是说它是一个跨张的一个请求伪造，对吧？close side request。Forgery。

然后他在这种夸张的一个请求伪造的话，它一个发生的条件是就说当我们用户在比如说一个A网站，对吧？你登录之后有这样子的一个登录的一个状态。然后的话。攻击者。呃，就是呃发送像这样子的一个用户。

发送呃一个恶意的一个网站。这个网站呢就比如说一个2意的一个链接，对吧？然后他访问这样子一跟链接的话，就访问到了我们的这样子的一个2还有恶E代码的一个网站B。然后这个网站B的话。

能够去劫持用户的一个浏览器。然后的话以用户的一个登录的一个状态或者是身份来发送啊，就是不是本人用，就是不是用户本人操作的一个请求。是。就说嗯。嗯。😊，然后的话CSF的话。

它是服务端没有对用户提交的一个数据进行一个这种随机值的一个校验，对吧？就是说他没有呃校验你的这样子的一个数据的一个来源。他的话没有对我们的1个HTP的一个请求包。就是常见的这种refi子弹是吧？

来进行一个校验。然后的话，攻击者可以利用这样子的1个CSF来利用用户的1个cookie信息来去呃进行一个伪造用户的一个请求来发送到指定的一个服务器。然后的话SSF的话。

它是服务器对用户提供的一个可控UL过于信任。就说呃SSF它是。呃，利用的就是。呃，用户就是利用的是服务器对用户提供的一1个UIL就是。过于信任。然后的话CSF它是呃。呃。是什么来着？突然忘记了。

而是对用户的这个这样子的一个请求是过于信任，他没有验证用户的一个就是一个呃身份。就说没有对用户提交的一个数据进行一个验证，然后对他的一个这种请求的话过于信任。所以的话会导致这样子的一个漏洞。
