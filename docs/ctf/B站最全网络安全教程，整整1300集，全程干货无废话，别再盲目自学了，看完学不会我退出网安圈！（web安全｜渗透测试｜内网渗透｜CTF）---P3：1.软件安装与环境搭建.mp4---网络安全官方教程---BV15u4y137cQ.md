# B站最全网络安全教程，整整1300集，全程干货无废话，别再盲目自学了，看完学不会我退出网安圈！（web安全｜渗透测试｜内网渗透｜CTF） - P3：1.软件安装与环境搭建.mp4 - 网络安全官方教程 - BV15u4y137cQ
...
啊，我们其实用的最多的也是啊这样子的一个终端吧。就我们如果呃对了，我忘了问了，问大家就是说大家。对lininux系统的一个操作熟悉吗？或者说了解吗？挺熟的。都都了解是吧。还行。还好。啊。
...
对。Yes。需要安装我们的这样的一个。基地库啊哎，这里怎么。ello没有。嗯。呃，我这里的话先跳过吧，因为这一个的话其实就是对那一个里面有一个靶场，它是需要使用到这样的一个验证码。
...
这样话我们直接执行这一个就可以了。他没加到他没有加到环境变量，我忘了勾选了吗？然后呃如果你发现你直接在下D里面查不了的话，你可以在这里加一个环境变量就可以了。



![](img/833c8cbaa1017d93b36bc41f476fb614_239.png)

就在一个pa里面。哎，那你加了呀。I行。啊，现在话可以了，可以看到。
...
但是他不会去进行一个使用，或者是呃或者是说是一个激活，就是一个破解。因为你因为它是一个收费的一个软件。然，这样的话我大概给大家介绍一下如何去啊。对他做一个破解以及。呃，使用吧。这样的话我把它放到了。

这是我自己的一个。我把它放到了一个百度网盘，就发给大家。嗯，大家可以做一个下载，呃，版本的话是1。7。3亿的一个版本。当然的话现在的话2。1点。2。1点多的一个版本也也是有了。然后的话在看你里面。

他其实。呃，已经有安装好了的一个软件在。大家如果使用咖里的话，可以直接去进行一个使用。而且它的一个版本的话，是如果你安装的是我这里给了一个新版本的一个镜像的话，它是1个2。1。07的一个版本。

比如说比这里的话是要新的，而且。功能的话是要多一点。就是说毕竟它的一个版本是要新一些嘛。然后这样的话我是给大家说的话是这个1。7。3亿的一个版本。然后大家下载好之后。就是这样子的一个。

这样子的四个文件就说其实它主要的其实它。本来里面只有这两个文件，这下面的话是我自己加的，我自己大概就。相当简单的学了。几个命令。就我们直接执行这样子的一个。BBS的一个脚本。

然后它就会打开我们这样子的1个BP。这样的话这里的话需要做一个破解。就如果你没有破解的话，它就会显示这样让你输入我们的一输入这样的一个license。这话我先取消。啊，首先的话对他做一个基本的一个破解。

我们首先需要运行这1个BP的1个load。这是一个破解的一个补定。执行之后的话，我们是这样子的一个界面，然后我们点这一个run。点击这个乱之的话，它就会启动这个。EPP这一个它的一个程序。

启动好之后的话，它让我他这里会让我们输入这样的一个license，我们直接复制这里的一个生成的一个license，复制粘贴啊，next next之后的话，它就会到这样子的一个界面。然后的话我们需要呃。

点击这里。这里有一个呃。点击之后的话会到这样子的一个界面。然后的话我们他这里会有提供给你一个request。然，我们直接oppy，然后把它粘贴到这里来。粘贴到这里来之后的话，它就会生成这样子的一个响应。

一个激活响应。我们直接全选复制复制，把它粘贴到这里来。Yes。粘贴之后的话，我们下一步next，然后finish之后就已经啊可以进入到我们的1个BP了。最后我们删掉。然后直接点下一步next。然后启动。

这个的话就已经可以把它擦掉，启动之后的话就是这样子的一个界面。如果熟悉的话，对这个工具熟悉的话。呃，大家对这个工具熟悉吗？就是呃也应该有去使用过吧。或者说没有使用过，但是应该也有听说过这一个工具吧。呃。

都没人说话吗？看来是都呃。没有使用过是吧？有使用过是吧？如果没有使用过的话，在呃下节课的话，我会给大家就是在讲HTP协议的时候，会给大家呃做一个它的一个基本的一个使用吧。就是它的一个抓包改包以及发包。

呃，我们使用到的最多的一些。诶功能。链接吗？呃，可以的，呃，等。等我们把这里讲完之后的话，下下课的时候，我再发到群里吧。那这里的话就已经呃激活成功了。结成功之后的话。嗯。我先把它拆掉。

就如果你激活成功中的话，如果你没有下面我这里写的这样两个文件的话，你会你首先你要去启动的话，你要去去点击这一个对吧？点击这一个，然后点这一个run。点击。呃，这里点击万造的话就到了这里。

我们需要下一步下一步，然后启动，对吧？所以说呃这里的话会有。多了这样的一个开启这样的一个步骤。所以我这里的话就啊因为难嘛。就偷懒的话就写了这样子的一个VPS的一个脚本。

这其实就是执行我们的这一个这一个的话就是用来启动。呃，启动这一个破解补丁，然后的话再启动这一个我们的这个应用程序。这是java相关的一个启动的一些呃执行的一个命令。然后的话就是。

直接在CMD里面执行我们的这个脚本，然后启动这一个。然后的话我们直接执行它执行它之后的话，它直接就。打开了嘛，就省去了中间的一些步骤，就其实主要就是为了。呃，稍微方便一点的，稍微方便一点吧。

然后这个这个的话我会发给大家，大家可以去呃下载一下，然后使用吧。然后大家对于今天的一个课程内容有没有什么疑问呢？呃，今天的一个课程内容的话大概就到这里了。呃，给大家5分钟的一个时间提问吧。

如果没有问题的话，呃，我们今天的一个课程内容的话就到此结束了。嗯，你要在虚拟机里面使用docker是吗？在虚拟机里面使用docker的话是可以的呀哦，你是说windows镜像吗？呃。

windows的一个多口的话，我嗯。我只在我自己的一个本机上面有使用过。但是嗯。我对这一个windows上面的一个docker的话不是很熟啊。

就是我之前的话是试过在window上面安装这样子的一个docker。然后的话。
