# B站最系统的护网行动红蓝攻防教程，掌握护网必备技能：应急响应／web安全／渗透测试／网络安全／信息安全 - P25：web安全-1.kali软件安装和环境搭建 - 跟小鱼学安全 - BV1SF411174M
...
然后这里的话呃我就不去不去使用这一个它应用商店里面自带的一个版本。呃，我使用的话是。

![](img/61417ab5981420ae052143983ced4d95_224.png)

在艾审的一个官网上面。下载了一个安装应用程序。呃，大家。要注意的话，这里这一个添加环境变量有没有必要去勾选一下，然后选择这个自定义的一个安装把这一些呃看自己的一个需求吧。像第一个的话，其实可点可不点。

就是一些呃它的一些pathon的一个说明的一个文档，一些文件。然后我们默认就可以了。然，next。这样的话我们可以自己选择一个安装的一个目录。呃说我这里的话。诶。

选择在C盘的一个program file，我们新进一个。先进一个python三的一个目录。然后我们选择安装在这一个目录下面，然后直接安装。Yeah。Yeah。嗯。嗯，这里有个问题。Yeah。Yeah。
